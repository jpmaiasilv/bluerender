...
const STAGE_TO_STEP_KEY: Record<JobStage, keyof ReturnType<typeof useLanguage>['messages']['videoGenerator']['loadingStatus']['steps']> = {
  uploading: 'preparing',
  sending: 'sending',
  rendering: 'rendering',
...
/** Video generation genuinely takes longer than image generation — this reuses the same visual language as the other tools' loading states, just with copy that doesn't overpromise a fast turnaround. */
export function VideoLoadingStatus({ stage }: Props) {
  const { messages } = useLanguage();
  const t = messages.videoGenerator.loadingStatus;
