/**
 * FOURTH authorized real execution — same floor plan, same CONFIRMED mask,
 ...
 * every private diagnostic the server itself wrote (vision detections, raw
 * Fill crop output, text verification, text correction), and reconstructs
 * (offline, no network, same pure functions the route uses) the crop sent
 * to FLUX and the recomposed-but-not-yet-wall-corrected canvas from the
 * real logged usefulAreaRect + the real saved raw Fill output.
 *
 * Run with: npx tsx scripts/inspect-humanized-floorplan-real-run-04.ts
 */
...
const OUTPUT_DIR = path.resolve(__dirname, '../test-output/humanized-floorplan-real-execution-04');
const PRIVATE_DIAGNOSTICS_ROOT = path.resolve(__dirname, '../private-diagnostics');
const SERVER_LOG_PATH = 'C:/Users/jpmai/AppData/Local/Temp/backend-dev.log';
...
async function main() {
  ...
  if (finalStatus.stage === 'complete' && finalStatus.result) {
    ...
    fs.writeFileSync(path.join(OUTPUT_DIR, '12-resultado-final.png'), imgBuffer);
    resultSaved = true;
  }

  const walletAfter = (await (await fetch(`${BASE_URL}/api/wallet`)).json()) as { balance: number };

  // --- Read back every private diagnostic the server wrote for this exact jobId ---
  ...
  const textCorrectionDiagDir = path.join(PRIVATE_DIAGNOSTICS_ROOT, 'humanized-floorplan-text-correction', jobId);
  ...
  if (textVerificationDiagnostics) fs.writeFileSync(path.join(OUTPUT_DIR, '10-text-verification-diagnostics.json'), JSON.stringify(textVerificationDiagnostics, null, 2));

  let textCorrectionDiagnostics: Record<string, unknown> | null = null;
  if (fs.existsSync(textCorrectionDiagDir)) {
    textCorrectionDiagnostics = JSON.parse(fs.readFileSync(path.join(textCorrectionDiagDir, 'report.json'), 'utf8'));
    for (const file of ['expanded-text-mask.png', 'before-restoration.png', 'after-restoration.png']) {
      const src = path.join(textCorrectionDiagDir, file);
      if (fs.existsSync(src)) fs.copyFileSync(src, path.join(OUTPUT_DIR, `11-correcao-${file}`));
    }
  }
  ...
  if (fs.existsSync(rejectionDiagDir)) {
    ...
    for (const file of fs.readdirSync(rejectionDiagDir)) {
      if (file.endsWith('.json')) {
        ...
        fs.copyFileSync(path.join(rejectionDiagDir, file), path.join(OUTPUT_DIR, `13-diagnostico-rejeicao-${file}`));
      }
    }
  }

  // --- Extract the EXACT usefulAreaRect this run computed, from the server's own log line for this jobId ---
  ...
  if (fs.existsSync(SERVER_LOG_PATH)) {
    ...
    while (true) {
      ...
      if (chunk.includes(jobId)) {
        ...
        if (rectMatch) usefulAreaRect = { x: Number(rectMatch[1]), y: Number(rectMatch[2]), width: Number(rectMatch[3]), height: Number(rectMatch[4]) };
  ...
  // --- Reconstruct (offline) the crop sent to FLUX and the recomposed-pre-correction canvas ---
  ...
  const summary = {
    ...
    hasTextCorrectionDiagnostics: textCorrectionDiagnostics !== null,
  ...
  if (textCorrectionDiagnostics) {
    console.log('\n=== TEXT CORRECTION DIAGNOSTICS ===');
    console.log(JSON.stringify(textCorrectionDiagnostics, null, 2));
  }
